...
public class ChangeRank implements CommandExecutor {
	...
	public boolean onCommand (CommandSender sender, Command cmd, String commandLabel, String[] args){
		if (cmd.getName().equalsIgnoreCase("changerank")){
			if (!(sender instanceof Player)){
				...
				if (settings.getConfig().getString("useinconsole") == "true"){
					...
					if (args.length == 3){
						...
						Bukkit.getServer().dispatchCommand(Bukkit.getConsoleSender(), "pex user " + args[0] + " group add " + args[2]);
						return true;
					}
					if (args.length >= 4){
						return false;
			...
			if (settings.getConfig().getString("useingame") == "true"){
				if(!(player.hasPermission("pex.changerank"))){
					...
					if (args.length == 3){
						...
						if (target != null) {
						    if (target.hasPermission("pex.immune")){
						    	...
						    	Bukkit.getServer().dispatchCommand(Bukkit.getConsoleSender(), "pex user " + args[0] + " group remove " + args[1]);
								Bukkit.getServer().dispatchCommand(Bukkit.getConsoleSender(), "pex user " + args[0] + " group add " + args[2]);
								player.sendMessage(ChatColor.AQUA + settings.getConfig().getString("rankchanged"));
								return true;
						    }
					}
					}
